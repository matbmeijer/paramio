...
Let's see a basic example showing how to use **Paramio**. Imagine we have a `parameters.yaml` file with all the projects parameters as the following. We could have it in any format (yaml, toml, json, etc.), the important aspect here is that the **dynamic variables** are defined with `f-string` formatting syntax:
